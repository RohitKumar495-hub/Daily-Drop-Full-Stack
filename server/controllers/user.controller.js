...
export async function verifEmailController(request , response) {
    try{
...
export async function verifyForgotPasswordOtp(request , response) {
    try{
        ...
        if(!email || !otp){
            return response.status(400).json({
                message : "Provide requried field email , otp",
                error : true,
                success : false
            })
        }
...
export async function resetPassword(request , response) {
    try{
        ...
        if(!email || !newPassword || !confirmPassword){
            return response.status(400).json({
                message : "Please Provide requried fields email , newPassword , confirmPassword",
                error : true,
                success : false
            })
        }
